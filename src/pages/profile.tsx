        ...
        <div className='m-auto lg:w-3/6 md:w-4/6 mt-16'>
          <div className='bg-blue-100 rounded-lg py-4 px-8'>
            ...
            <div className=' py-3 px-6 rounded-lg flex items-center'>
              <div>
                <input type='text' disabled={!editing} defaultValue={profileData.profile.username || profileData.user.name} name='username' className='border-0 outline-none bg-transparent mb-3 text-xl'/>
                <Image src={profileData.user.image || ProfilePlaceholder} alt='Profile avatart' width={100} height={100} className='rounded-full'/>
              </div>
              <textarea disabled={!editing} defaultValue={profileData.profile.bio || 'Tell the world what kind of traveler you are!'} name='bio' className='bg-slate-50 p-2 rounded-lg w-full border-0 resize-none'></textarea>
            </div>
              ...
              <div className='flex flex-col space-y-3'>
                  ...
                  <form onSubmit={handleProfileEdit} id='profileForm'>
                    ...
                    <div className='flex items-center space-x-2'>
                      ...
                      <input type='radio' value={'MILES'} disabled={!editing} defaultChecked={profileData.profile.distanceUnits === 'MILES'} id='milesFormat' name='distanceUnits' className='rounded-md text-sky-400 border-gray-300'/>
                      <label htmlFor='milesFormat'>Miles (mi)</label>

                      <input type='radio' value={'KILOMETERS'} disabled={!editing} defaultChecked={profileData.profile.distanceUnits === 'KILOMETERS'} id='kilometersFormat' name='distanceUnits' className='rounded-md text-sky-400 border-gray-300'/>
                      <label htmlFor='kilometersFormat'>Kilometers (km)</label>

                      <input type='radio' value={'BANANAS'} disabled={!editing} defaultChecked={profileData.profile.distanceUnits === 'BANANAS'} id='bananasFormat' name='distanceUnits' className='rounded-md text-sky-400 border-gray-300'/>
                      <label htmlFor='bananasFormat'>Bananas (🍌)</label>
                    </div>

                    <div className='flex items-center space-x-2'>
                      <label>Date format: </label>
                      <input value={'MONTH'} type='radio' id='monthFirst' disabled={!editing} defaultChecked={profileData.profile.dateFormat === 'MONTH'} name='dateFormat' className='rounded-md text-sky-400 border-gray-300'/>
                      <label htmlFor='monthFirst'>Month/Day <span className='text-gray-400 text-sm'>(5/23)</span></label>
                      <input value={'DAY'} type='radio' id='dayFirst' disabled={!editing} defaultChecked={profileData.profile.dateFormat === 'DAY'} name='dateFormat' className='rounded-md text-sky-400 border-gray-300'/>
                      <label htmlFor='dayFirst'>Day/Month <span className='text-gray-400 text-sm'>(23/5)</span></label>
                    </div>

                    <div className='flex items-center space-x-2'>
                      <label>Time format:</label>
                      <input type='radio' id='12' disabled={!editing} value={'12'} defaultChecked={profileData.profile.timeFormat === 'TWELVE'} name='timeFormat' className='rounded-md text-sky-400 border-gray-300'/>
                      <label htmlFor='12'>12h <span className='text-gray-400 text-sm'>(1:30pm)</span></label>
                      <input type='radio' id='24' disabled={!editing} value={'24'} defaultChecked={profileData.profile.timeFormat === 'TWENTYFOUR'} name='timeFormat' className='rounded-md text-sky-400 border-gray-300'/>
                    ...
                    <div className='flex items-center space-x-2'>
                      <input type='checkbox' disabled={!editing} value={'true'} defaultChecked={profileData.profile.commentsNotification} id='commented' name='commentsNotification' className='border-gray-300 rounded-md text-sky-400'></input>
                      <label htmlFor='commented'>Someone commented on your itinerary</label>
                    </div>

                    <div className='flex items-center space-x-2'>
                      <input type='checkbox' disabled={!editing} value={'true'} defaultChecked={profileData.profile.remindersNotification} id='reminders' name='remindersNotification' className='border-gray-300 rounded-md text-sky-400'></input>
                      <label htmlFor='reminders'>Reminders about your upcoming trip</label>
                    </div>

                    <div className='flex items-center space-x-2'>
                      <input type='checkbox' disabled={!editing} value={'true'} defaultChecked={profileData.profile.collaboratorJoinedNotification} id='friendJoined' name='collaboratorNotification' className='border-gray-300 rounded-md text-sky-400'></input>
